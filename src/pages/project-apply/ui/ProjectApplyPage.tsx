  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-[1040px] px-5 py-10">
        ...
        <div className="mx-auto mt-10 max-w-[640px] space-y-6">
          <section className="rounded-md border border-line bg-page shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
            <div className="border-b border-line px-6 py-5">
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                <span className="text-[13px] text-pale">등록일 : {formatPostedAtDisplay(project.postedAt)}</span>
              </div>
              <h1 className="mt-4 text-[28px] font-bold leading-[1.35] text-ink">
                『{project.title}』 프로젝트 지원하기
              </h1>
              <p className="mt-3 text-[14px] leading-6 text-dim">
                프로젝트 정보를 확인한 뒤 지원 내용을 입력해주세요.
              </p>
            </div>

            <div className="grid gap-0 sm:grid-cols-2 lg:grid-cols-4">
              <SummaryMetric label={formatBudgetLabel(project)} value={formatProjectBudget(project)} />
              <SummaryMetric label="예상기간" value={`${project.averagePeriodDays}일`} />
              <SummaryMetric label="지원자수" value={`${project.applicants}`} />
              <SummaryMetric label="마감일정" value={formatDeadlineMetric(project.deadlineLabel)} />
            </div>
          </section>

          <section className="rounded-md border border-line bg-page px-6 py-6 shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
          ...
          <section className="rounded-md border border-line bg-page shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
            <div className="border-b border-line px-6 py-5">
              <h2 className="text-[22px] font-bold text-ink">지원서 작성</h2>
              ...
              <form className="px-6 py-6" onSubmit={(event) => void handleSubmit(event)}>
                ...
                {project.type === 'budget' ? (
                  <div className="mt-6 space-y-5">
                    <FormField label="예상금액">
                      ...
                      />
                      <HelperText>프리모아 이용료 10%가 포함된 금액입니다.</HelperText>
                    </FormField>

                    <FormField label="예상일수">
                      <UnitInput
                ...
                <div className="mt-6 flex gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex h-11 flex-1 items-center justify-center rounded-md bg-[#ff7a18] text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {submitState.isSubmitting ? '제출 중...' : '지원하기'}
                  </button>
                </div>
              </form>
            )}
...
function SummaryMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-[#f7f7f7] px-4 py-4 text-center sm:border-r sm:last:border-r-0">
  ...
  return (
    <div className="rounded-sm border border-line bg-[#fafafa] px-4 py-4">
      ...
      <div className="mt-3 space-y-2 text-[13px] leading-6 text-dim">
        <p>
          <span className="font-medium text-ink">모집 마감일</span> {formatDeadlineValue(project)}
        </p>
        <p>
          <span className="font-medium text-ink">예상 킥오프</span> {project.kickoffSchedule}
        </p>
        <p>
          <span className="font-medium text-ink">고용형태</span> {formatEmploymentLabel(project.type)}
        </p>
        <p>
